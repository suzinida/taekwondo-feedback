...
function playSetup() {
  ...
  // 모바일 환경 대응
  demoVideo.play().catch(err => console.log('시범 영상 재생:', err));
  studentVideoSetup.play().catch(err => console.log('학생 영상 재생:', err));
  
  isPlayingSetup = true;

  // 동기화 로직 (50ms 주기로 더 자주 확인)
  ...
  syncInterval = setInterval(() => {
    if (isPlayingSetup) {
      ...
      // 오차가 0.5초 이상이면 동기화
      if (Math.abs(studentVideoSetup.currentTime - targetTime) > 0.5) {
...
function playBoth() {
  ...
  // 모바일 환경 대응
  demoVideoFeedback.play().catch(err => console.log('시범 영상 재생:', err));
  studentVideoFeedback.play().catch(err => console.log('학생 영상 재생:', err));
  ...
  syncInterval = setInterval(() => {
    if (isPlayingFeedback) {
      ...
      // 오차가 0.5초 이상이면 동기화
      if (Math.abs(studentVideoFeedback.currentTime - targetTime) > 0.5) {
